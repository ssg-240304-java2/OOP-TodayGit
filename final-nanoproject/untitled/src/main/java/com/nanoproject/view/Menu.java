...
import com.nanoproject.controller.MenuController;
...
public class Menu {
    ...
    public void run() {
        ...
        while (!isPayable) {
            ...
            scanner.nextLine();
            switch (input) {
                case 1, 2, 3 -> menuController.addItem(new BeverageDTO(items.get(index).getName(), items.get(index).getPrice(), askHot()));
                case 4, 5 -> menuController.addItem(new DessertDTO(items.get(index).getName(), items.get(index).getPrice(), askHeat()));
                case 6 -> menuController.showBasket();
                case 7 -> menuController.deleteItem(getItemNo());
                case 8 -> menuController.clearBasket();
                case 9 -> isPayable = menuController.isPayable();
                default -> System.out.println("잘못된 입력입니다.");
            }
            System.out.println();
        }
        payment.run(member, menuController.getTotalPrice());
    }
    ...
    private int getItemNo() {
        int itemNo = -1;
        if (!menuController.showBasket()) return itemNo;
        while (true) {
            System.out.print("몇 번 메뉴를 삭제하시겠습니까? : ");
            itemNo = scanner.nextInt();
            if (0 < itemNo && itemNo <= menuController.getItemCount()) break;
            System.out.println("잘못된 입력입니다.");
        }
        return itemNo;
    }
